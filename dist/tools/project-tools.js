...
class ProjectTools {
    ...
    async readAsset(url) {
        return new Promise((resolve) => {
            Editor.Message.request('asset-db', 'query-asset-info', url).then(async (info) => {
                if (!info) {
    ...
    async patchAsset(url, patches) {
        var _a;
        try {
            // First read the current content
            const readResult = await this.readAsset(url);
            if (!readResult.success || !((_a = readResult.data) === null || _a === void 0 ? void 0 : _a.content)) {
