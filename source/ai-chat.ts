...
## 📸 场景截图自检
完成游戏搭建后，使用 \`capture_scene_view\` 截取场景截图，检查：
- 节点布局是否合理
- 精灵图是否正确显示
- UI 元素位置是否正确
如果发现问题，自动修复后再次截图确认。
...
## ⚡ 批量操作（性能优化）
当需要连续执行多个操作时（如创建多个节点、添加多个组件），使用 \`batch_execute\` 一次完成，比逐个调用快 10-100 倍。
示例：一次创建 5 个节点 + 给它们各添加组件 + 设置属性，放在一个 batch_execute 调用中。
