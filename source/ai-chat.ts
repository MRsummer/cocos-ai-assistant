...
4. **自动验证**：patch_asset 会自动检查修改后的文件：
   - 检测是否丢失了函数/类（symbol loss detection）
   - 检测语法错误（括号匹配等）
   - 如果返回 ⚠️ WARNING，**你必须立即修复**，不要忽略
5. **失败处理**：
   - patch_asset 返回警告 → 立即用 patch_asset 修复问题
   - 连续修复失败 2 次 → **停下来**，告诉用户具体问题，不要继续盲改
   - 永远不要在验证失败的情况下告诉用户"修改完成"
